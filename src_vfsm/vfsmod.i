# 1 "/Users/carpena/UF Dropbox/Rafael Munoz-Carpena/Work/Claire_France/vfsm4.6.1/src_vfsm/vfsmod.f"
      PROGRAM VFSMOD
...
C     Last Updated: 01/2026 v4.6.1.2                                           C
...
c--------------Check output instability (non-convergence, NaN) and if so restart
            ...
            DO 22 I = 1, N
              IF (Q0(I) .NE. Q0(I)) THEN
                IF(ISCR.NE.2) THEN
                    write(*,'(/,A32,f8.2,I4,E12.4,/)')'ERROR: NaN in Q0.
     *                    TIME,I, Q0(I)
                    STOP
                  ELSE
...
c---- (IWQ=1) Pesticide leaching, degradation and remobilization ------------
      IF(IWQ.GT.0) THEN
            DO 50 JJ=1,IWQ
c------ Leaching, CDE analytical solution, 3rd type BC (Huang & van Genu
...
c-------Output message at end of program -----------------
      ...
      IF(ISCR.EQ.0) THEN
        WRITE(*,*)
        WRITE(*,*)'...FINISHED...','VFSMOD v4.6.1.2 01/2026'
        WRITE(*,*)
      ENDIF
