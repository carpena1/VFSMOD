# 1 "/Users/carpena/UFL Dropbox/Rafael Munoz-Carpena/Work/Claire_France/vfsm4.6.1/src_vfsm/wqpest.f"
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      COMMON/WQ3/DGKREF(10),FC,DGPIN(10),DGML,DGT(366),DGTHETA(366),DGLD
      COMMON/CDE1/DGMfFd(10),DGMfFp(10),DGMfF(10),DGMmld(10),DGMmlp(10),
      
      DIMENSION DGMRES0P(10),DGMI(10),DGMO(10),DGMOD(10),DGMOP(10),DGMf(
     ...
     &      DGMRESDAREA(10), DGMRESIAREA(10),DGMSurfAREA(10),DGMmlAREA(1
      CHARACTER*200 LINE
      CHARACTER*3 SNDGDAY 
      CHARACTER*1 SIMOB 
...
c-------------Mechanistic mass balance eq. - (DEFAULT)
                DELTAP=100.D0*MIN(VINL+VKD(JJ)*SMIN,PDSED/100*SMIN*VKD(J
...
c-------------Chen eq. (California)
     ...
     &          2.17D0*ICAT+0.02D0*PDQ*ICAT-0.0003D0*PDQ*PDSED)**2.D0
            END SELECT
            IF (DELTAP.GT.100.D0.OR.PDQ.GE.100.d0.OR.VOUT.LE.0.d0) DELTA
...
c---------Check approximation error (%) on sediment bonded pesticide and issue warning if >1%
            ...
            errp=100.D0*(DGMI(JJ)-(DGMml(JJ)+DGMfSED(JJ)+DGMfFb)-DGMO(JJ
...
c---------  First, calculate mass for degradation after loss to leaching (drainage from Os to FC)
            DGMml(JJ) = DGMmlp(JJ) + DGMmld(JJ) * FC/OS
...
c-------and water (VOUT)assuming linear equilibrium based on Kd (VKD(JJ)).
        IF(VOUT.GT.0.D0) THEN
            DGMOP(JJ)=(DGMO(JJ)*SMOUT*VKD(JJ))/(VOUT*1000.D0+SMOUT*VKD(J
...
3525  FORMAT(100('-'),/,1x,'Pesticide mass balance, degradation & remobi
...
c-----prepare lines for mass balance with multiple compounds in a single
     *DUM(JJ): array containing values for each compound (JJ=1,WQ)
c-----DESCR: Units and descriptor for valueIFMT
c-----: 0 for outputs with no days or imob variable (mg), 1,2 for ND
...
c-----      4,5 for NDGDAY and IMOB (mg/m2), 
      ...
      SELECT CASE (IFMT)
        CASE (1)
          WRITE (SNDGDAY, '(I3)') NDGDAY
          LINE=TRIM(LINE) // ' mg   = ' // TRIM(DESCR) // SNDGDAY // ' d
        CASE (2)
          WRITE (SIMOB, '(I1)') IMOB
        ...
        CASE (4)
          WRITE (SNDGDAY, '(I3)') NDGDAY
          LINE=TRIM(LINE) // ' mg/m2= ' // TRIM(DESCR) // SNDGDAY // ' d
        CASE (5)
          WRITE (SIMOB, '(I1)') IMOB
