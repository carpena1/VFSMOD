...
## Unreleased (since 4.6.2.1)

**(09/2026)**

### Maintenance & Documentation

- Removed the indirect recursion func -> qgaus -> func in the shallow water table Green-Ampt infiltration (gasubwt.f), which is not allowed in Fortran 77. All four integrals of the unsaturated conductivity vKuns now use the new routine qgausk, and qgaus no longer has the ieq=1 (Kuns) option, so a single routine integrates vKuns and the call graph has no cycle. Results are bit-identical.
- Removed the unused functions SCONC and ERF and the subroutine QGAUSSSORB (cde.f). SCONC and QGAUSSSORB lost their callers when the sorbed-mass quadrature was replaced in 4.6.2.1; ERF was never called and shadowed the Fortran 2008 intrinsic of the same name. Results are bit-identical.
- Renamed the COMMON/PAR/ variable MAXISTER to MAXITER in shapef.f to match all other program units (not referenced in shapef.f; no effect on results).
- New regression sample sampleWTpond (sampleWT with Ks lowered to 5.0e-6 m/s) added to check_vfsm.sh. It covers ponding over a shallow water table, a branch of gasubwt.f that no previous sample executed.
...
- New automatic node-count selection when N=-1 in the .ikw (inputs.f). The previous VL-based formula (N increasing with strip length) is replaced by a Courant-number rule N=2*nint((23*CR+1.4)/2)+1 with a short-strip node cap min(N,2*nint(VL)+1), clamped to [5,101]. A MOC-based study (PGPAR_fit_results/Case1_MOC, optimize_N_VL.py) scoring the runoff depth across the mesh vs the analytical method of characteristics (Eq.37, rising+recession limbs) found the mesh that simultaneously minimizes spatial error AND Petrov-Galerkin output oscillation is N~=26*CRR, essentially independent of VL and of physics (verified n=0.007-0.4); over-refining (N>>26*CRR) makes BOTH error and oscillation worse. The Jaber-Mohtar dynamic time step never binds for N<=101, so CRR=CR here and the rule reads CR directly. The cap is a node limit (not a fixed dx) so very short strips still run (VL=0.25m -> N=5, dx=0.06m). Runs with N>0 are unaffected.
- Optional per-node depth time-series for code development/verification against the analytical MOC (vfsmod.f, inputs.f). When IELOUT=2 (a development-only value; production runs use IELOUT=0 or 1) VFSMOD writes output/<base>.oxt with a header line "N DX" then "TIME h(1..N)" at each print step, and also prints the small .ohy elemental table exactly as IELOUT=1. Moved off the production path (was IELOUT=1) so that IELOUT=0 and IELOUT=1 runs never create this file, which can be large for fine meshes (N x print-steps); production behavior for IELOUT=0,1 is unchanged.
- Output hydrograph smoothing of the .ohy OUTFLOW column (kwwrite.f, vfsmod.f). Each of the 100 printed outflow values is a block-average of the NWRITE=NDT/NPRINT solver steps; for stiff, high-kinematic-wave-number cases (e.g. dense vegetation / very low Manning n on short strips) this block-averaged hydrograph can retain a residual low-frequency Petrov-Galerkin oscillation. A centered 5-point median filter is now applied to the printed OUTFLOW series and renormalized to preserve the raw outflow volume (OSUMFLOW), so the .osm water balance and every other .ohy column are unchanged. Validated against the analytical MOC (WRR-1993 Case1 and a stiff VL=0.15m, n=0.05 case): removes the oscillation (plateau CV 24%->7%) with 0.00% change in outflow volume and no harm to clean or dynamic hydrographs. Reporting only (the solver is untouched); implemented as a buffered flush (OHYFLUSH/MEDIAN5) after the time loop.

### Bug Fixes

- Revised hydfilter output-file reset on the adaptive-Courant (Qo=NaN) restart (inputs.f, vfsmod.f) when a runtime instability triggers the CR-reduction rerun.
- Removed the hard low-flow noise filter in FLOW (flow.f) to suppress Petrov-Galerkin oscillations. It introduced a discontinuity in q(X) that negatively affected the Picard iteration in ultra-low-flow cases (tiny areas (spots), rainfall, Ks=0)
- Sediment Delivery Ratio 0/0=NaN guard (outmass.f) written in .osp for cases with no inflow.
- Sediment-wedge NaN guard for very small incoming sediment concentration (step3.f). With Ci~0 in the .isd (e.g. in an SPOTMOD case with Ci=1.9E-10 g/cm3) the Einstein equilibrium slope Set (proportional to load^0.28) collapses below the bed slope Sc, so Se=Set-Sc<0 and the wedge-growth term Yt=dsqrt(2/gammasb*fi*GSI_C*Se*DTG+Ytold^2) (and the 1/Se in Xt) takes the square root of a negative number, returning NaN that propagated through Y(t),X1(t),X2(t),L(t),gso in .og1 and was picked up by outmass into .osp and .owq. Guarded on Se<=0 to route all incoming sediment through the wedge to the lower filter (no depositional wedge can form when the equilibrium slope is gentler than the bed), mirroring the existing NTRCAP=1 pass-through branch; the negligible load is then trapped by Tollner in the suspended-sediment zone as usual. Cases with Se>0 are untouched.
- Skip CDE soil leaching when there is no infiltration (cde.f). For VKS=0 (no infiltration, event infiltration F=0) T=F/OS=0, and the pulse concentration C1=DGCIN2 is 0/0, so the pulse concentration and all six leaching mass-balance terms (soil profile mfF, mfFd, mfFp and mixing layer mfml, mfmld, mfmlp) came out NaN. Guarded on F<=0 to force C1=0, set the six leached/mixing-layer masses to 0, and skip the depth-profile table. Infiltrating runs (F>0) are untouched (bit-identical SPOTMOD Row_deltas).
...
- CDE secant (chord) retardation for Freundlich isotherms (cde.f, 2026-06). The retardation factor R in the Huang & van Genuchten (1995) analytical leaching solution is now the secant slope of the isotherm, R = 1 + (rho/theta)*Kf*C^(Nf-1), instead of the tangent slope R = 1 + (rho/theta)*Nf*Kf*C^(Nf-1). For a self-sharpening loading front (favorable isotherm, Nf<1) the front travels at the chord slope by the Rankine-Hugoniot mass-balance shock condition, so the secant is the physically correct, mass-conserving retardation; it reduces exactly to the linear Kd at Nf=1 (bit-identical results for all linear-isotherm inputs). The fixed-point iteration on R/C1 is unchanged apart from dropping the Nf factor in both the initial estimate and the loop.
- Consistent sorbed-mass accounting in the CDE profile and mixing layer (cde.f, 2026-06). The nonlinear quadrature (qgausssorb) over the leaching profile is replaced by mfFp = (R-1)*mfFd for the soil profile and mfmlp = (R-1)*mfmld for the mixing layer, using the same secant R that governs transport. This makes the total infiltrated mass exact, mfF = R*mfFd = dQ.mdi, removing the +33% mfF over-count and the associated -6% to -15% closure error that the tangent R produced for samplePF (Freundlich, Nf=0.85); single-event closure for samplePF is now +0.82% (the irreducible residual from the linear-Kd split used by the dP trapping step). For Freundlich the two changes above must ship together: b1 mfsed with the tangent R would worsen closure; b1 mfsed with the secant R restores it.
...
**(05/2026)** — changes introduced for SPOTMOD integration
...
- Added non-linear Freundlich sorption isotherm option (IKD=2) as an  alternative to the existing linear isotherms (IKD=0 direct Kd; IKD=1  via Koc and %OC). The Freundlich isotherm s=Kf*C^Nf is implemented  consistently across all sorption-dependent processes in the pesticide  component: (a) soil leaching CDE analytical solution (cde.f): the  concentration-dependent retardation factor R=1+Nf*Kf*C^(Nf-1)*rho_b/theta_s  is linearized at the average event flux concentration C1 via a  fixed-point iteration ensuring self-consistent R and C1; (b) incoming  pesticide phase partitioning (dissolved/sorbed) uses an effective  linearized Kd_eff=Kf*C^(Nf-1) evaluated at the estimated inlet  concentration; (c) outflow solid/liquid phase partitioning uses  Kd_eff at the estimated outflow concentration; (d) inter-event surface  residue porewater/sorbed split is solved via Newton-Raphson iteration  on the implicit equation Cd*Vw+Kf*Cd^Nf*DGMsoil=DGMRES, with the  linear solution as initial guess ensuring rapid convergence; (e) all  other partitioning uses linearized Kd_eff at the relevant local  concentration. For linear isotherms (IKD=0,1) VKF=VKD and VKN=1 are  set internally so all Freundlich branches reduce exactly to the  original linear code (backwards compatible). New input format for  IKD=2 in .iwq file: "2  Kf  Nf" where Kf is in L^Nf/kg and Nf is the  dimensionless Freundlich exponent (0<Nf<=1 for typical isotherms).  New COMMON/WQ1/ variables VKF(10) and VKN(10) added to all 5 files  that declare this block (vfsmod.f, inputs.f, cde.f, wqpest.f,  outmass.f). Reference: Muñoz-Carpena, R., S. Reichenberger, R. Sur.  2026. Effect of linear and Freundlich isotherms in the prediction with  VFSMOD of pesticide residue remobilization from vegetative filter  strips. SETAC Europe 2026, Maastrich, Netherlands.
...
- Added VKF(10) and VKN(10) to COMMON/WQ1 in multspcalc.f (was missing from the initial v4.6.2 update, making this file inconsistent with the other 5 files that declare this block).

### Maintenance & Documentation

- Minor update to outmass.f: expanded COMMON/WQ1 declaration to include  the new VKF(10) and VKN(10) arrays for Freundlich Kf and Nf parameters.
- Updated output format for sorption coefficients in .owq and CDE  output: linear isotherm now labelled "Linear sorption coeff. (Kd)";  Freundlich now printed as single line  "Non-linear Freundlich (Kf,Nf)= Kf L^Nf/Kg, Nf (-)".
...
- (In preparation) Adding Freundlich sorption isotherm option to .iwq file. To ensure backwards compatibiity, this is added as an additional option to the second line of iwq (IKD=2 values read are Kd and Freundlich exponent Nf; IKD=3 values read are Koc, OC, and Freundlich exponent Nf)
...
- Added Multispecies (parent-metabolites) degradation (multspcalc.f). User can define up to 10 chemical species with parent metabolite relations through an adjacency matrix in .iwq, after setting the number of chemicals (IWQ>1) in .ikw. Individual chemicals to run together can also be run by only providing a -1 diagonal on the adjacency matrix. The problem is solved efficiently as an eigenvalue problem of the ODE system with a numerical approximation.
- Added multispecies leaching, remobilization and final mass balance.
