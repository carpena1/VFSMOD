...
- Added KPG=2 option (inputs.f): when KPG=2, PGPAR1-4 are read from inputs/<base>.ipg instead of being computed, used by the PGPAR-fitting/optimization driver. Research/instrumentation hook; KPG=0/1/3 behavior is unchanged.
- Added optional per-node depth time-series output (inputs.f, vfsmod.f): when IELOUT=1, VFSMOD writes output/<base>.oxt with a header line "N DX" followed by "TIME h(1..N)" at each print step, for comparison with the analytical MOC depth field. Gated on IELOUT; default IELOUT=0 runs are byte-for-byte unaffected.
