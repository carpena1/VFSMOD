...
- Bug fix: Sediment Delivery Ratio 0/0=NaN guard (outmass.f) written in .osp for cases with no inflow.
- Bug fix: skip CDE soil leaching when there is no infiltration (cde.f). For VKS=0 (no infiltration, event infiltration F=0) T=F/OS=0, and the pulse concentration C1=DGCIN2 is 0/0, so the pulse concentration and all six leaching mass-balance terms (soil profile mfF, mfFd, mfFp and mixing layer mfml, mfmld, mfmlp) came out NaN. Guarded on F<=0 to force C1=0, set the six leached/mixing-layer masses to 0, and skip the depth-profile table. Infiltrating runs (F>0) are untouched (bit-identical SPOTMOD Row_deltas).
